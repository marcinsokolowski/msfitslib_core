...
struct cValue {
   ...
   cValue() : x(0),y(0),z(-1000), err(0.00) {}
   cValue( double _x, double _y, double _z=0, double _v=0 ) : x(_x), y(_y), z(_z), v(_v), err(0.00) {}
...
double interpolate( cValue* file_values, int count, double x, int bRe=1 );
