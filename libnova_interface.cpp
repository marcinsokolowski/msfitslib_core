...
//   ln_get_solar_rst( JD, &observer, &sun_info );
   ...
   struct ln_equ_posn sun_position;
   ln_get_solar_equ_coords(JD, &sun_position);
   if( bVerb > 0 ){
      printf("Sun equatorial coordinates (ra,dec) = (%.4f,%.4f) [deg]\n",sun_position.ra,sun_position.dec);                 
   }
